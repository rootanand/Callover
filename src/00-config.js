...
;(function (CO) {
  ...
  */
  ...
  /* An opener that is BOTH a salutation and a plausible set of initials.
     ...
     Two and three letters only. An advocate carries one, two or three
     initials, so MRS can be M.R.S. and SMT can be S.M.T., but THIRU and
     ADVOCATE are never anybody's initials and are not listed.

     ORDERED LONGEST FIRST, and that is load-bearing rather than tidy.
     Alternation in JavaScript takes the first branch that matches, so with MR
     before MRS the opener of "MRS.KANNAN" would be read as M.R. and the name
     left as "S.KANNAN" — a different person. T21-09 locks the ordering.

     A SEPARATOR IS REQUIRED — a dot or a space. Without one the opener matches
     inside ordinary words: ADV swallows the front of "ADVOCATE", leaving a
     phantom advocate called OCATE, and SRI does the same to "SRIKANTH", which
     is a real Tamil name and one of the firm's own. A prefix that has been
     run into the following word is not a prefix, it is a coincidence. */
  CO.AMBIGUOUS_OPENER = /^(MRS|SMT|ADV|SRI|TMT|MR|MS|DR|SR)[.\s]+(?=[A-Z])/;
