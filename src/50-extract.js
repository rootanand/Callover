...
;(function (CO) {
  ...
  function passA_table(page, table, profile, roles, ctx, file) {
    ...
    /* WHICH ROW IS THE HEADING — asked of the content, never assumed.

       A long list runs over many pages and the column headings are printed
       once, on the first. Every continuation page opens straight into a
       matter. Dropping row 0 regardless therefore threw away the FIRST MATTER
       ON EVERY PAGE AFTER THE FIRST — silently, since the row was read
       perfectly and then discarded. Found on a real 12-page list where
       "R.P.122/2023 L.Purusothamman through M/s.E.Ganesh" was the top row of
       page 3 and simply never appeared.

       A heading has no case number in its case-number cell. That is the whole
       test, and it is the one thing that cannot be true of a real row. */
    const iCaseCol = roles.map((x, i) => x.role === 'caseNumber' ? i : -1).filter(i => i >= 0)[0];
    const rowText = (row, c) => {
      const cell = row[c];
    ...
    const looksLikeHeading = row => {
      if (!row) return false;
      if (iCaseCol == null) return false;
      const c = rowText(row, iCaseCol);
      if (/\d/.test(c)) return false;              // carries a number: it is a matter
